...
export const updateBigQueryMockData = onCall(async (request) => {
    ...
    try {
        ...
        if (loginAccreditedDate || discussionDate) {
            ...
            if (loginAccreditedDate) setQuery += `, first_login_at = '${loginAccreditedDate}'`;
            if (discussionDate) setQuery += `, first_discussion_post_at = '${discussionDate}'`;
        ...
        if (fundingComplete !== undefined || wwowStarted !== undefined || startDate || reserveDate) {
            ...
            if (fundingComplete !== undefined) coreUpdates += `, funding_plan_status = '${fundingComplete ? 'Complete' : 'Pending'}'`;
            if (wwowStarted !== undefined) coreUpdates += `, wwow_orientation_started_at = ${wwowStarted ? 'CURRENT_TIMESTAMP()' : 'NULL'}`;
            if (startDate) coreUpdates += `, program_start_date = '${startDate}'`;
            if (reserveDate) coreUpdates += `, reserve_date = '${reserveDate}'`;
        ...
        throw new HttpsError('internal', 'BQ Write Failed ' + JSON.stringify(error));
    }
});
...
export const manualSyncBQtoFirestore = onCall(async (request) => {
    ...
    try {
        ...
        for (const student of rows) {
            ...
            const registeredCourses = validCourses.some((c: any) => c.course_registration_status === 'Registered');
            ...
            const hasDiscussionPost = validCourses.some((c: any) => c.first_discussion_post_at !== null);
            ...
            if (hasDiscussionPost && student.program_start_date) {
                ...
                censusCheck = validCourses.some((c: any) => c.first_discussion_post_at && new Date(c.first_discussion_post_at.value).getTime() <= censusDateMs);
            }
    ...
    } catch (error) { throw new HttpsError("internal", "BigQuery Sync Failed."); }
});
